...
;; persisted-preset compatibility. Generated linuxptp configuration emits
;; `slaveOnly`: Ubuntu 22.04 LTS ships linuxptp 3.1.1, whose config parser has
;; no `clientOnly` key, while 4.x still accepts `slaveOnly` as a deprecated
;; alias (config.c auto-converts it), so `slaveOnly` is the only key that is
;; valid on every supported target.
...
(define (default-params-for-role role)
  (case role
    ...
    [(listener)
     (make-params #:priority1 248 #:gm-capable 0 #:slave-only 1)]
    [(boundary)
     ;; A BC port set must be able to hold GM on downstream ports, so it is
     ;; not client-only and keeps the profile priority (248).
     (make-params #:priority1 248 #:gm-capable 1 #:slave-only 0)]
    [else (make-params)]))
...
(define (params->conf p #:role [role "listener"] #:iface [iface #f]
  ...
  (define boundary? (eq? role-symbol 'boundary))
  (define effective (apply-role-constraints p role-symbol))
  (define iface-display
    (cond
      [(pair? ifaces) (string-join (map ~a ifaces) ",")]
      [iface (~a iface)]
      [else "auto"]))
  (string-append
   "# ptp4l.conf generated by gPTP Studio\n"
   (format "# role: ~a   iface: ~a\n" (role-name role-symbol) iface-display)
   (if boundary?
       (format "# boundary clock: ~a ports (port 1 = first listed = upstream)\n"
               (length ifaces))
       "")
   "# profile: IEEE 802.1AS (gPTP)\n"
   "\n[global]\n"
   (kv "gmCapable" (gptp-params-gm-capable effective))
   (kv "slaveOnly" (gptp-params-slave-only effective))
   ...
   ;; A BC shares one discipline source across ports: jbod stays off so the
   ;; downstream side is driven by the clock recovered upstream (phc2sys -a -r
   ;; keeps port PHCs aligned). Per-port PHC islands (jbod=1) are out of scope.
   (if boundary? (kv "boundary_clock_jbod" 0) "")
